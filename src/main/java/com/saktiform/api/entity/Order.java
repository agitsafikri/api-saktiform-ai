...
public class Order {
    ...
    @Column(name = "status_ekspor")
    Boolean statusEkspor;
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "last_handle_by", insertable = false, updatable = false)
    private Account handleByAccount;

    @Column(name = "last_handle_by")
    private Long lastHandleBy;
